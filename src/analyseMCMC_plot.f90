...
subroutine pginitl(colour,file,whiteBG)
  ...
  if(whiteBG.ge.1) then
     ...
     if(file.le.1 .or. use_PLplot) then    ! png/PLplot: create white background
  ...
  if(use_PLplot) then
     call pgsvp(0.08,0.9,0.17,0.99) ! Default viewport size (?)
  else
     call pgsvp(0.08,0.95,0.1,0.87) ! Default viewport size (?)
  end if
