...
if [[ ${#} -ne 3 && ${#} -ne 5 ]]; then
    ...
    echo -e "\n  Syntax:"
    echo -e "    code_version.sh  <CMake base dir>  <f90 output file>  <PLplot?>  <Fortran compiler name>  <Compiler flags>"
    echo -e "    code_version.sh  <CMake base dir>  <f90 output file>  <PLplot?>  # For release versions\n"
    ...
    PLPLOT=${3}                       # Using PLplot? (true if "yes")
    
    if [[ ${#} -eq 5 ]]; then
	COMPILER=${4}                   # Compiler name
	COMPILER_FLAGS=${5}             # Compiler flags
    fi
    ...
    if [ ${PLPLOT} == 'yes' ]; then
	echo "!> \file code_version_plplot.f90  Source file automatically generated by CMake to report the code version used." > ${F90FILE}
    else
	echo "!> \file code_version_pgplot.f90  Source file automatically generated by CMake to report the code version used." > ${F90FILE}
    fi
    echo "" >> ${F90FILE}
